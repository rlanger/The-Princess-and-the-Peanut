...
-(id) initWithView:(NavView *)aView
{
	...
	if( (self=[super init]) ) {
        ...
        //DOORS
        doors = [[NSMutableArray alloc] initWithCapacity:1];

        CCTexture2D *castleTexture = [[CCTextureCache sharedTextureCache] addImage:@"castle.png"];
        Door *castle = [Door npcWithTexture:castleTexture andModel:self andType:OBJECT andConnection: @"Castle"];
        castle.position = CGPointMake((size.width/2), size.height/2);
        [doors addObject:castle];
        [view addChildToMid:castle];
